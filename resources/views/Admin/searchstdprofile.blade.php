...
<div class=search2>
    <form action="/searchstdprofile/search" method="GET" role="search">
        <div class="input-group">
            ...
            <a href="/searchstdprofile">&emsp;<button  class="refreshbtn" type="button" title="Refresh page"><ion-icon name="repeat-outline"></ion-icon></button></a>
        </div>
                    @if(!empty($successMsg))
...
@foreach($deta as $detaa)
  ...
  <td>{{$detaa->std_matric}}</td>
    ...
    <td><a href="/displaystdprofile/{{ $detaa->id }}"><button type="button" style="background-color: white; border: 1px solid white;" > <ion-icon name="eye-outline"></ion-icon></button></a> &emsp;<a href="/searchstdprofile/{{ $detaa->id}}"><button type="button" style="background-color: white; border: 1px solid white;" onclick="return confirm('Are you sure?This record and it`s details will be permanantly deleted!')"><ion-icon name="trash-outline"></ion-icon></button></a></td>
  </tr>
  @endforeach
...
<br><br>
<a href="createstdprofile" class=" mt-1">
